...
class LeftPanelMixin:
    ...
    def setup_left_panel(self):
        ...
        # 1. TOOLBOX FRAME (Upper Section)
        upper_sidebar_frame = tk.Frame(self.left_panel, bg="#0f172a")
        upper_sidebar_frame.grid(row=0, column=0, sticky="nsew")

        # 1. TOOLBOX SECTION
        section_toolbox = tk.Label(
            upper_sidebar_frame, text=t("toolbox.add_nodes"), font=("Segoe UI", 9, "bold"),
            fg="#94a3b8", bg="#0f172a"
        )
        section_toolbox.pack(pady=(15, 5), padx=10, anchor="w")
        
        # Add Node Buttons
        # Define grouped node list by categories
        node_groups = [
            ("INTERAÇÃO E ENTRADA", [
        ...
        buttons_container = tk.Frame(upper_sidebar_frame, bg="#0f172a")
        ...
        for group_title, nodes in node_groups:
            # Stylish section header divider
            group_label = tk.Label(
                scrollable_frame, text=group_title, font=("Segoe UI", 8, "bold"),
                fg="#38bdf8", bg="#1e293b", anchor="w", padx=10
            )
            group_label.pack(fill="x", pady=(12, 4), ipady=3)
            group_label.bind("<MouseWheel>", _on_mousewheel)
            
            for name, type_key, color in nodes:
                btn = tk.Button(
        ...
        lower_sidebar_frame.grid(row=1, column=0, sticky="nsew")
        
        divider = tk.Frame(lower_sidebar_frame, bg="#1e293b", height=1)
        divider.pack(pady=(0, 5), padx=15, fill="x")
        
        log_header = tk.Frame(lower_sidebar_frame, bg="#0f172a")
        log_header.pack(fill="x", padx=10, pady=(0, 5))
        
        log_label = tk.Label(
            log_header, text=t("toolbox.debug_console"), font=("Segoe UI", 9, "bold"),
            fg="#94a3b8", bg="#0f172a"
        )
        log_label.pack(side="left")
        
        expand_btn = tk.Button(
            log_header, text="🗖 " + t("toolbox.expand_log"), font=("Segoe UI", 8, "bold"),
            bg="#1e293b", fg="#38bdf8", activebackground="#334155", activeforeground="#38bdf8",
            bd=0, padx=5, pady=2, cursor="hand2", command=self.expand_log_console
        )
        expand_btn.pack(side="right")
        
        self.log_text = tk.Text(
            lower_sidebar_frame, bg="#1e293b", fg="#e2e8f0", bd=0, 
